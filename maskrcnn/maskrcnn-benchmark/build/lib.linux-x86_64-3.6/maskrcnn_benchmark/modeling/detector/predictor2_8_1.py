...
class Predictor(nn.Module):
    ...
    def __init__(self, cfg=None, select_num=10, class_num=4, side=False, random_select=False):
        ...
        self.conv_obj = nn.Conv2d(2048, 2048, 7)
        self.relu_obj = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        ...
        obj2 = self.relu_obj(self.conv_obj(obj))
        # obj2 = self.relu_obj(self.conv_obj(obj))

        if not self.random_select:
            glob2 = glob.expand(obj.shape[0], 256, 7, 7)
            obj2 = obj2.expand(obj.shape[0], 2048, 7, 7)
            x = torch.cat((obj2, glob2), dim=1) # (N, 2048+256, 7, 7)
            # Select objects
