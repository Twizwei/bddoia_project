...
class Predictor(nn.Module):
    ...
    def forward(self, x):
        ...
        scores = self.selector(glob, bbox, scale_ratio) # (N, 1, 1, 1)
        ...
        if not self.random_select:
            if self.select_num < idx.shape[0]: # in case that too few objects detected
                score_thresh = 0.5 * (scores_sorted[self.select_num - 1] + scores_sorted[self.select_num]).squeeze()
                one_hot = self.sigmoid((scores - score_thresh)) >= 0.5
                obj = (scores * one_hot.type_as(scores)) * obj # (N, 256, 14, 14)
                obj = obj[torch.nonzero(one_hot[:,0])[:,0]][:self.select_num] # (select_num, 256, 14, 14)
            else:
        ...
        # x = self.drop(self.relu_fc1(self.fc1(x)))
        tmp = torch.flatten(x) # (select_num * 256)

        # print(tmp.shape)
        x = self.drop(self.relu2(self.fc2(tmp))) # (64)
        # x = self.drop(self.relu_fc2(self.fc2(tmp)))
