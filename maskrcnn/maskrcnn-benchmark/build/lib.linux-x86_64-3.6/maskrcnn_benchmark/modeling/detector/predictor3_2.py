...
class Predictor(nn.Module):
    ...
    def __init__(self, cfg=None, select_num=5, class_num=4, side=False):
        ...
        self.conv_obj1 = nn.Conv2d(2048, 1024, 3)
        self.relu_obj1 = nn.ReLU(inplace=True)
        self.conv_obj2 = nn.Conv2d(1024, 512, 3)
        self.relu_obj2 = nn.ReLU(inplace=True)
        self.conv_obj3 = nn.Conv2d(512, 256, 3)
        self.relu_obj3 = nn.ReLU(inplace=True)
        ...
        # FC
        self.conv1 = nn.Conv2d(1024, 1024, 4)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(1024, 1024, 4)
        self.relu2 = nn.ReLU(inplace=True)
        self.drop = nn.Dropout(p=0.2) 
        self.fc1 = nn.Linear((select_num)*(256+1024), 512)
        self.relu1 = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        ...
        glob2 = self.relu_glob1(self.conv_glob1(glob))
        glob2 = self.relu_glob2(self.conv_glob2(glob2)) # (1, 256, 1, 1)
        glob2 = torch.squeeze(glob2)
        glob2 = torch.unsqueeze(glob2, 0) # (1, 256)
        ...
        obj = self.relu_obj3(self.conv_obj3(obj)) # (N, 256, 1, 1)
        ...
        glob2 = glob2.expand(obj.shape[0], 256)
        ...
        del scores, idx, scores_logits, x, obj2, glob2

        
        #glob = torch.cat((torch.cuda.FloatTensor([[0,0,0,0]]), glob),dim=1) # (1, 4+256)
        glob = self.relu1(self.conv1(glob))
        glob = self.relu2(self.conv2(glob)) # (1, 1024, 1, 1) 
        glob = torch.squeeze(glob)
        glob = torch.unsqueeze(glob, 0) # (1, 1024)
        glob = glob.expand(obj.shape[0], 1024)
        x = torch.cat((obj, glob), dim=1) # (N, 256+1024)
        del obj, glob
