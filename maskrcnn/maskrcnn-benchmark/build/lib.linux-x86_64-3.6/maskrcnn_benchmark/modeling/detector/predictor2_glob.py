...
class Predictor(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = torch.flatten(x) # (1, 256*7*7)
        # x = self.drop(self.relu1(self.fc1(x))) # (1, 256)
        
        x = torch.flatten(self.avgpool_glob2(x)) # (1, 256)
        tmp = self.drop(self.relu2(self.fc2(x))) # (1, 64)
        x = self.fc3(tmp)
        if self.side:
