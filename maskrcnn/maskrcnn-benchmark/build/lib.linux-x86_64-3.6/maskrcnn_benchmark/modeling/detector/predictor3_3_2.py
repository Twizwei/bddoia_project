...
class Predictor(nn.Module):
    ...
    def __init__(self, cfg=None, select_num=10, class_num=4, side=False, random_select=False):
        ...
        self.fc1 = nn.Linear(260+260, 256)
        self.relu1 = nn.ReLU()
    ...
    def forward(self, x):
        ...
        glob = torch.cat((torch.cuda.FloatTensor([[[[0]],[[0]],[[0]],[[0]]]]), glob), dim=1) # (1, 260, 1, 1)
        glob = glob.expand(obj.shape[0], 260, 1, 1)
        
        x = torch.cat((obj, glob), dim=1) # (select_num, 260+260, 1, 1)
        x = torch.squeeze(x) # (select_num, 260+260)
        del obj, glob
        
        x = self.drop(self.relu1(self.fc1(x))) # (select_num, 260)
