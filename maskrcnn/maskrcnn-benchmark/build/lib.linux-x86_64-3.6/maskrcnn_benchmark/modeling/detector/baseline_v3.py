...
class Baseline(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        from .predictor3_7_2 import build_predictor
    ...
    def forward(self, images, targets=None, get_feature=True): #TODO: Remember to set get_feature as False if there are any changes later
        ...
        if self.roi_heads:
            hooker_pooler = SimpleHook(self.roi_heads.box.feature_extractor.pooler)
            _, result, _ = self.roi_heads(features, proposals, targets)
            result, idx = result # result is a list of box, idx is a list of index, length is the same as the batch size.
            x = hooker_pooler.output.data # (N, 1024, 14, 14)
            # print(x.shape)

            xx = []
            tmp = 0
            for i in range(b_size):
                xx.append(x[tmp: tmp + bbox_num[i]][idx[i]])# list of (N, 1024, 14, 14), N is the number of detected boxes.
                tmp = bbox_num[i]
            ...
            for i in range(b_size):
                ...
                # hook_selector = SimpleHook(self.predictor.sftmax)
                if self.side:
